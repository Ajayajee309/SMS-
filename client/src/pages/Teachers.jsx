...
export default function Teachers() {
  ...
  const [formData, setFormData] = useState({
    employeeId: '', name: '', department: '', email: '', phone: '', designation: '', subjects: ''
  });
  ...
  const openAddModal = () => {
    ...
    setFormData({ employeeId: '', name: '', department: '', email: '', phone: '', designation: '', subjects: '' });
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (teacher) => {
    ...
    setFormData({
      employeeId: teacher.employeeId || '', name: teacher.name || '', department: teacher.department || '',
      email: teacher.email || '', phone: teacher.phone || '', designation: teacher.designation || '', subjects: teacher.subjects || ''
    });
    setIsModalOpen(true);
  };
  ...
  const saveTeacher = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert('Error saving teacher. Employee ID might be duplicate.');
    }
  };
              ...
              {teachers.length === 0 ? (
                ...
                teachers.map((t) => (
                  <tr key={t.id} className="hover:bg-blue-50/50 transition-colors">
                    <td className="p-4 text-sm font-medium text-gray-900">{t.employeeId}</td>
                    <td className="p-4 text-sm font-medium text-blue-600">
                      {t.name}
                      <div className="text-xs text-gray-500 font-normal">{t.email}</div>
                    </td>
                    <td className="p-4 text-sm text-gray-700">
                      <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded-md text-xs font-medium">{t.department}</span>
                    </td>
                    <td className="p-4 text-sm text-gray-700">{t.designation || 'N/A'}</td>
                    <td className="p-4 flex items-center justify-end gap-2">
                      <button onClick={() => openEditModal(t)} className="admin-only text-gray-500 hover:text-emerald-600 p-1.5 rounded-md hover:bg-emerald-50 transition-colors">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => deleteTeacher(t.id)} className="admin-only text-gray-500 hover:text-red-600 p-1.5 rounded-md hover:bg-red-50 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white z-10">
              <h3 className="text-xl font-bold text-gray-800">{currentTeacher ? 'Edit Teacher' : 'Add New Teacher'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="admin-only text-gray-400 hover:text-gray-600"><X className="w-6 h-6" /></button>
            </div>
            
            <form onSubmit={saveTeacher} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Employee ID *</label>
                  <input required name="employeeId" value={formData.employeeId} onChange={handleInputChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input name="designation" value={formData.designation} onChange={handleInputChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  ...
                  <input name="subjects" value={formData.subjects} onChange={handleInputChange} className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
                </div>
              </div>
